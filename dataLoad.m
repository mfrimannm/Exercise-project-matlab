...
for i = 1:N
        ...
        if (file(i, 1) >= 10) && (file(i, 1) <= 60) 
            ...
            fprintf('The temperature in row %d did not meet the condtions and was removed.\n',i)
        end
        %if "Growth rate" condition is met, insert valid row in data. Else false element becomes 0.
        if file(i, 2) > 0
            data(i, 2) = file(i, 2);
        else
            fprintf('The growth rate in row %d did not meet the condtions and was removed.\n',i)
        end
        %if "Bacteria" condition is met, insert valid row in data. Else false element becomes 0.
        if (file(i, 3) >= 1) && (file(i, 3) <= 4)
            data(i, 3) = file(i, 3);
        else
            fprintf('The bacteira in row %d did not meet the condtions and was removed.\n',i)
        end
        for j = 1:3
